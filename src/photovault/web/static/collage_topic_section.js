(async function(){
  ...
  async function loadPickedScope(topicId,sectionId=""){
    const query=new URLSearchParams({picks:"1",limit:"100"});
    if(sectionId)query.set("section",sectionId);
    const handoff=await api(`/api/culling/${encodeURIComponent(topicId)}/photos?${query.toString()}`);
    ...
    const sectionLabel=sectionId
      ? ([...sectionSelect.options].find(option=>option.value===sectionId)?.textContent||"the selected section")
      : "All topic photos";
    $("status").textContent=`${handoff.total} picked photos loaded from ${sectionLabel}.`;
  }
