document.querySelector("#export-package")?.addEventListener("click",async()=>{
  ...
  const assets=[...new Set((doc.elements||doc.frames||[]).map(x=>x.photo_id||x.asset_id).filter(Boolean))];
  document.querySelector("#status").textContent="Building ChatGPT design package…";
  try{
    const r=await fetch("/api/collage/design-packages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({asset_ids:assets,page_spec:doc.page_spec||{},mode:"improve_existing",style:doc.style||"organic",current_document:doc})});
