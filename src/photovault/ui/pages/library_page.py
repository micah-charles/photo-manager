...
def build_library_page(owner: object, layout: object, tables: dict[str, QTableWidget]) -> None:
    ...
    quarantine_button = QPushButton("Plan rejected for quarantine…")
    quarantine_button.setToolTip("Prepare a reversible quarantine plan for selected rejected files; execution remains a separate confirmed step.")
    quarantine_button.clicked.connect(owner._plan_selected_rejected_quarantine)
    actions.addWidget(quarantine_button)
    ...
    metadata_actions.addWidget(QLabel("Assign selected:"))
    owner.library_assign_event = QComboBox()
    owner.library_assign_event.setMinimumWidth(140)
    metadata_actions.addWidget(owner.library_assign_event)
    assign_event_button = QPushButton("Event")
    assign_event_button.clicked.connect(owner._assign_selected_event)
    metadata_actions.addWidget(assign_event_button)
    owner.library_assign_tag = QComboBox()
    owner.library_assign_tag.setMinimumWidth(120)
    metadata_actions.addWidget(owner.library_assign_tag)
    assign_tag_button = QPushButton("Tag")
    assign_tag_button.clicked.connect(owner._assign_selected_tag)
    metadata_actions.addWidget(assign_tag_button)
    owner.library_assign_place = QComboBox()
    owner.library_assign_place.setMinimumWidth(120)
    metadata_actions.addWidget(owner.library_assign_place)
    ...
    remove_tag_button.clicked.connect(owner._remove_selected_tag)
    metadata_actions.addWidget(remove_tag_button)
    clear_place_button = QPushButton("Clear Place")
    clear_place_button.clicked.connect(owner._clear_selected_place)
    metadata_actions.addWidget(clear_place_button)
    metadata_actions.addStretch(1)
    layout.addLayout(metadata_actions)
