...
def build_photo_viewer_page(owner: object, layout: object) -> None:
    ...
    organisation.addWidget(QLabel("Organise this photo:"))
    owner.viewer_event_action = QComboBox()
    owner.viewer_event_action.setMinimumWidth(150)
    organisation.addWidget(owner.viewer_event_action)
    event_button = QPushButton("Event")
    event_button.clicked.connect(owner._assign_viewer_event)
    organisation.addWidget(event_button)
    owner.viewer_tag_action = QComboBox()
    owner.viewer_tag_action.setMinimumWidth(130)
    organisation.addWidget(owner.viewer_tag_action)
    tag_button = QPushButton("Tag")
    tag_button.clicked.connect(owner._assign_viewer_tag)
    organisation.addWidget(tag_button)
    owner.viewer_place_action = QComboBox()
    owner.viewer_place_action.setMinimumWidth(130)
    organisation.addWidget(owner.viewer_place_action)
    place_button = QPushButton("Place")
    place_button.clicked.connect(owner._assign_viewer_place)
    organisation.addWidget(place_button)
    owner.viewer_person_action = QComboBox()
    owner.viewer_person_action.setMinimumWidth(130)
    organisation.addWidget(owner.viewer_person_action)
    person_button = QPushButton("Person")
    person_button.clicked.connect(owner._assign_viewer_person)
    organisation.addWidget(person_button)
    organisation.addStretch(1)
    layout.addLayout(organisation)

    album_row = QHBoxLayout()
    album_row.addWidget(QLabel("Add to album:"))
    owner.viewer_collection_action = QComboBox()
    owner.viewer_collection_action.setMinimumWidth(240)
    album_row.addWidget(owner.viewer_collection_action)
    album_button = QPushButton("Add")
    album_button.clicked.connect(owner._add_viewer_to_collection)
    album_row.addWidget(album_button)
